...
public abstract class AbstractPartitionedLimiter<ContextT> extends AbstractLimiter<ContextT> {
    ...
    public AbstractPartitionedLimiter(Builder<?, ContextT> builder) {
        ...
        builder.partitions.values().stream().map(Partition::getPercent).reduce(0.0, Double::sum) <= 1.0,
    ...
    extends AbstractLimiter.Builder<BuilderT> {
        ...
        public Limiter<ContextT> build() {
            return this.hasPartitions() && !partitionResolvers.isEmpty() ? new AbstractPartitionedLimiter<ContextT>(
            this) {
